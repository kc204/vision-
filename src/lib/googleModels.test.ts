...
test("resolveGoogleModel matches configured base model when only latest alias is available", async (t) => {
  const apiUrl = "https://generativelanguage.googleapis.com/v1beta";
  const apiKey = "entitled-latest-only";
  const models = ["gemini-1.5-pro", "imagen-3.0-generate-001"];
  ...
  const fetchMock = t.mock.method(globalThis, "fetch", async (input: RequestInfo) => {
    ...
    return new Response(
      JSON.stringify({ models: [{ name: "models/gemini-1.5-pro-latest" }] }),
      { status: 200, headers: { "content-type": "application/json" } }
    );
  });
  ...
  assert.equal(resolved, "gemini-1.5-pro");
  assert.equal(fetchMock.mock.calls.length, 1);
});
